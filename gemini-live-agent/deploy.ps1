...
gcloud run deploy $SERVICE_NAME --image $IMAGE --platform managed --region $REGION --allow-unauthenticated --memory 1Gi --cpu 1 --timeout 3600 --min-instances 0 --max-instances 10 --set-env-vars "GOOGLE_GENAI_USE_VERTEXAI=FALSE,GOOGLE_API_KEY=AIzaSyBfkkYMRFpDcUYTl2sivsGPCofcy6SlUKA,GOOGLE_CLOUD_PROJECT=$PROJECT_ID,GOOGLE_CLOUD_LOCATION=$REGION" --project=$PROJECT_ID --quiet
